...
| CI | `.github/workflows/ci.yml` | One job, `validate`, runs on every PR and on every push to `main`. It runs `uv sync --locked`, the validation-gate commands below as separate steps in the same order, and an import smoke test (`BING_WEBMASTER_API_KEY=dummy uv run python -c "import mcp_server_bwt.main"`). `validate` is the required-check name for the `main` ruleset (#11), so never rename the job. The workflow steps mirror `validation.commands` in `.ai/agentic.config.json`, so change them together. There are no `paths:` filters, because a required check has to report on every PR. |
