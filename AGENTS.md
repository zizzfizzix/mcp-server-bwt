...
| CI | `.github/workflows/` | Only the release-please workflow runs today (on push to `main`). TODO: no validation CI yet (#11), so the validation gate below is the only check. Once required checks exist, release PRs need the `RELEASE_PLEASE_TOKEN` secret, because PRs opened with the default token don't trigger other workflows. |
