...
| CI | `.github/workflows/` | Only the release-please workflow runs today (on push to `main`). TODO: no validation CI yet (#11), so the validation gate below is the only check. The workflow opens release PRs with the `RELEASE_PLEASE_TOKEN` repo secret (it falls back to the default token, whose PRs don't trigger other workflows), so future required checks run on release PRs too. |
