...
The server keeps each list it downloads for 5 minutes (see `BING_WEBMASTER_CACHE_TTL` below). Repeat calls with the same arguments, including later pages, are served from that copy instead of downloading the whole list again. So a list can be up to 5 minutes old, even right after a tool such as `add_blocked_url` changes it.
